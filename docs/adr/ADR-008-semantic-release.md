...
- update version fields in source files
- rely on local release commands
- allow manual version bumps

Artifact versioning is injected **at build time** via Go `ldflags`:
...
- Requires Node tooling in CI (for semantic-release, alongside Go)
...
### File-based versioning (e.g. hardcoded `version.go`)
