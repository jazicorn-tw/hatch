...
#
# - Fast fail for missing required tools; Colima not-running is a warning only
...
#   DOCTOR_REQUIRE_COLIMA_RUNNING=1 -> fail if Colima is installed but not running (overrides local-settings)
...
_ls_max_colima_mem=""
_ls_max_colima_cpus=""
_ls_require_colima_running="false"
if [[ -f "${_LS_FILE}" ]] && command -v jq >/dev/null 2>&1; then
  ...
  _ls_require_colima_running="$(jq -r '.doctor.requireColimaRunning // "false"' "${_LS_FILE}" 2>/dev/null || echo "false")"
fi

# Convert JSON bool "true"/"false" → 1/0
_ls_require_colima_running_flag="0"
[[ "${_ls_require_colima_running}" == "true" ]] && _ls_require_colima_running_flag="1"
...
REQUIRE_COLIMA_RUNNING="${DOCTOR_REQUIRE_COLIMA_RUNNING:-${_ls_require_colima_running_flag}}"
...
if command -v colima >/dev/null 2>&1; then
  ...
  if colima status >/dev/null 2>&1; then
    ...
    COLIMA_RUNNING=0
    if [[ "${REQUIRE_COLIMA_RUNNING}" == "1" ]]; then
      die "Colima is not running. Start it with: colima start (or set doctor.requireColimaRunning=false in local-settings.json to downgrade to a warning)"
    else
      warn "Colima is not running (not required for Go dev). Start it when you need containers: colima start"
    fi
  fi
else
  HAS_COLIMA=0
...
if ! docker info >/dev/null 2>&1; then
  ...
  elif [[ "${OS}" == "Darwin" && "${HAS_COLIMA}" == "1" ]]; then
...
if [[ -n "${DOCKER_CPUS}" ]]; then
  ...
  if [[ "${DOCKER_CPUS}" =~ ^[0-9]+$ ]]; then
    if (( DOCKER_CPUS < MIN_CPUS )); then
      warn_or_die "Docker reports ${DOCKER_CPUS} CPUs; recommended >= ${MIN_CPUS} CPUs."
    fi
  fi
else
  ...
  if [[ "${OS}" == "Darwin" ]]; then
    ...
    if [[ "${need_suggest}" == "1" ]]; then
      if [[ "${COLIMA_AUTO_SCALE}" == "1" ]]; then
        ...
        if [[ "${JSON_MODE}" != "1" ]]; then
          say ""
          say "💡 Tip: Increase Colima resources:"
          say "   colima stop"
