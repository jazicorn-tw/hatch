...
1. **Commitizen** (`cz`) as the optional Conventional Commits validator — warns and skips if not installed
...
3. A **guarded hook implementation** that:
   - validates commit messages via `cz check` (when `cz` is available)
   - falls back to regex-based validation when `cz` is not installed
...
- When `cz` is installed, validation is performed by:

  ```bash
  cz check --commit-msg-file <file>
  ```

- When `cz` is **not** installed, the hook warns and falls back to regex validation
- Developers are encouraged (but not required) to use:
...
- Commitizen is optional but recommended for the best authoring experience
